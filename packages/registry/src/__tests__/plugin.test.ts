  ...
  describe('isHookPlugin', () => {
    test('should return true for valid plugin', () => {
      const plugin: HookPlugin = {
        ...
        version: '1.0.0',
        events: ['PreToolUse'],
        apply: async () => ({ success: true, pluginName: 'test', pluginVersion: '1.0.0' })
      };
...
describe('Plugin Error Classes', () => {
  describe('PluginValidationError', () => {
    test('should create error with correct message and properties', () => {
      const error = new PluginValidationError('test-plugin', 'name', 'Invalid name format');
      
      expect(error.name).toBe('PluginValidationError');
      expect(error.message).toBe('Plugin test-plugin: name - Invalid name format');
  ...
  describe('PluginExecutionError', () => {
    test('should create error with correct message and properties', () => {
      ...
      const error = new PluginExecutionError('test-plugin', 'Execution failed', originalError);
  ...
  describe('PluginConfigurationError', () => {
    test('should create error with config field', () => {
      const error = new PluginConfigurationError('test-plugin', 'Invalid timeout value', 'timeout');
      
      expect(error.name).toBe('PluginConfigurationError');
      expect(error.message).toBe('Plugin test-plugin configuration error: Invalid timeout value');
      expect(error.pluginName).toBe('test-plugin');
      expect(error.configField).toBe('timeout');
    });
    
    test('should create error without config field', () => {
      const error = new PluginConfigurationError('test-plugin', 'Invalid configuration');
      
      expect(error.name).toBe('PluginConfigurationError');
      expect(error.message).toBe('Plugin test-plugin configuration error: Invalid configuration');
      expect(error.pluginName).toBe('test-plugin');
