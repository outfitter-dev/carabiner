...
describe('Edge cases and error handling', () => {
  ...
  test('should handle malformed input deterministically', async () => {
    ...
    await expect(runTestHook(handler, undefined as unknown as Record<string, unknown>)).rejects.toBeDefined();
    ...
    for (const input of toleratedInputs) {
      await expect(runTestHook(handler, input as Record<string, unknown>)).resolves.toBeUndefined();
    }
  });
});
