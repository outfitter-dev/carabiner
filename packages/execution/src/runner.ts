...
import { StdinProtocol, TestProtocol, createProtocol } from '@outfitter/protocol';

import { HookExecutor, type ExecutionOptions } from './executor';
...
export class HookRunner {
   ...
   */
  async run(handler: HookHandler): Promise<never | void> {
    const protocol = this.createProtocol();
    const executor = new HookExecutor(protocol, this.options);
 ...
 */
export function getExecutionMetrics(timeRange?: { start: number; end: number }) {
  if (timeRange) {
 ...
 */
export function hasRecentFailures(withinMs = 60000): boolean {
