 ...
 */
const GitSafetyConfigSchema = z.object({
  ...
  confirmationMessage: z.string().default('This is a potentially dangerous git operation. Are you sure?'),
 ...
 */
function matchesPatterns(command: string, patterns: string[]): { matched: boolean; pattern?: string } {
...
export const gitSafetyPlugin: HookPlugin = {
  name: 'git-safety',
  version: '1.0.0',
  description: 'Prevents dangerous git operations like force pushes and hard resets',
  ...
  apply(context: HookContext, config: Record<string, unknown> = {}): PluginResult {
    ...
    if (context.event !== 'PreToolUse' || !('toolName' in context) || context.toolName !== 'Bash') {
      return {
        success: true,
    ...
    // Get command and current working directory
    const bashContext = context as any; // Type assertion for toolInput access
    const command = bashContext.toolInput?.command as string;
    const cwd = bashContext.cwd as string || process.cwd();
    ...
    for (const rule of safetyConfig.customRules) {
      ...
      if (ruleMatch.matched) {
        if (rule.severity === 'warn') {
          ...
          return createWarningResult(this, gitCommand, rule.message, safetyConfig);
        }
      }
    }
  ...
  async init(): Promise<void> {
    try {
      ...
      console.warn('[GitSafety] Git not available - plugin may not work correctly:', error);
    }
  },
