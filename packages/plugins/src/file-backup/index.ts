...
const FileBackupConfigSchema = z.object({
  ...
  namingStrategy: z.enum(['timestamp', 'numbered', 'date']).default('timestamp'),
  ...
  maxFileSize: z.number().min(0).default(10 * 1024 * 1024), // 10MB
 ...
 */
function generateBackupFilename(originalPath: string, strategy: string, counter?: number): string {
...
async function getExistingBackups(
  ...
  try {
    ...
    for (const file of files) {
      ...
      switch (strategy) {
        case 'timestamp':
          isBackup = file.startsWith(`${name}.`) && file.endsWith(ext) && /\.\d+\./.test(file);
          break;
        case 'numbered':
          isBackup = file.startsWith(`${name}.backup.`) && file.endsWith(ext);
          break;
        case 'date':
          isBackup = file.startsWith(`${name}.`) && file.endsWith(ext) && /\.\d{4}-\d{2}-\d{2}_/.test(file);
          break;
        default:
          isBackup = file.startsWith(`${name}.backup`) && file.endsWith(ext);
      }
...
async function cleanupOldBackups(
  ...
  const existing = await getExistingBackups(filePath, backupDir, config.namingStrategy);
  ...
  if (existing.length > config.maxBackups) {
    ...
    for (const backup of toRemove) {
      try {
        ...
        console.warn(`[FileBackup] Failed to remove backup ${backup.path}:`, error);
      }
    }
  }
    ...
    for (const backup of existing) {
      if (backup.created < cutoff) {
        try {
          ...
          console.warn(`[FileBackup] Failed to remove expired backup ${backup.path}:`, error);
        }
      }
    }
  }
...
async function createBackup(
  ...
  try {
    ...
    if (config.includePatterns.length > 0 && !matchesPatterns(filePath, config.includePatterns)) {
    ...
    const existing = await getExistingBackups(filePath, backupDir, config.namingStrategy);
    const backupFilename = generateBackupFilename(filePath, config.namingStrategy, existing.length + 1);
...
export const fileBackupPlugin: HookPlugin = {
  ...
  async apply(context: HookContext, config: Record<string, unknown> = {}): Promise<PluginResult> {
    ...
    const fileContext = context as any; // Type assertion for toolInput access
    ...
    for (const filePath of filePaths) {
      ...
      if (backupResult.success) {
        ...
        if (backupConfig.logOperations) {
          console.error(`[FileBackup] Failed to backup ${filePath}: ${backupResult.error}`);
        }
      }
    }
