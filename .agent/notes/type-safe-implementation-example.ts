...
export const createSessionId = (value: string): SessionId | never => {
  if (!value || value.length < 1) throw new Error('Invalid session ID');
  return value as SessionId;
};

export const createFilePath = (value: string): FilePath | never => {
  if (!value || !value.includes('/')) throw new Error('Invalid file path');
  return value as FilePath;
};

export const createCommandString = (value: string): CommandString | never => {
  if (!value || value.length > 10000) throw new Error('Invalid command');
...
export interface ToolInputMap {
  ...
  readonly LS: { readonly path: FilePath; readonly ignore?: ReadonlyArray<string> };
  readonly TodoWrite: {
...
export type HookContext = BaseHookContext & (
  | {
      readonly event: { readonly type: 'PreToolUse'; readonly tool: ToolName };
  ...
  | {
      readonly event: { readonly type: 'PostToolUse'; readonly tool: ToolName };
      readonly toolInput: ToolInputMap[ToolName];
  ...
  | {
      readonly event: { readonly type: 'SessionStart' | 'Stop' | 'SubagentStop' };
      readonly toolInput?: never;
      readonly toolResponse?: never;
      readonly userPrompt?: never;
...
const SessionIdSchema = z.string().min(1).transform((val): SessionId => val as SessionId);
const FilePathSchema = z.string().min(1).transform((val): FilePath => val as FilePath);
const CommandStringSchema = z.string().min(1).max(10000).transform((val): CommandString => val as CommandString);
const ToolContentSchema = z.string().max(1000000).transform((val): ToolContent => val as ToolContent);
...
export class HookBuilder<TState extends HookBuilderState = { hasEvent: false; hasHandler: false }> {
...
export class TypeSafeHookRegistry {
  ...
  register(entry: HookRegistryEntry): void {
    ...
    this.hooks.set(key, [...existing, entry].sort((a, b) => 
      (b.priority ?? 0) - (a.priority ?? 0)
    ));
  }
  
  getHandlers<T extends HookEvent>(event: T): HookHandler<HookContext & { event: T }>[] {
    const key = `${event.type}${'tool' in event ? `:${event.tool}` : ''}`;
    const entries = this.hooks.get(key) || [];
    
    return entries
      .filter(entry => entry.enabled !== false)
      .map(entry => entry.handler) as HookHandler<HookContext & { event: T }>[];
  }
}
